...
def analyze_contract_images(images_data: list[tuple[bytes, str]]) -> Optional[ContractAnalysisResult]:
    """
    Analyze multiple contract images using Gemini Vision.
    images_data: list of tuples (image_bytes, mime_type)
    """
    ...
    if len(images_data) == 1:
        return analyze_contract_image(images_data[0][0], images_data[0][1])
...
**작업 1: 텍스트 추출 (OCR)**
여러 장의 계약서 이미지에서 모든 텍스트를 정확히 추출하세요.
이미지 순서대로 텍스트를 연결하고, 원본 형식(줄바꿈, 번호 등)을 최대한 유지하세요.
...
응답은 반드시 한국어로 작성하세요."""
    ...
    try:
        content_parts = []
        for idx, (img_bytes, mime_type) in enumerate(images_data):
        ...
        content_parts.append(system_prompt + f"\n\n위 {len(images_data)}장의 계약서 이미지를 순서대로 분석해주세요.")
