...
def highlight_text_with_risks(extracted_text: str, risk_clauses: list[RiskClause]) -> str:
    ...
    for idx, clause in enumerate(sorted(risk_clauses, key=lambda x: len(x.original_text), reverse=True), 1):
        ...
        if safe_original and safe_original in highlighted:
            ...
            highlight_html = f'''<span class="risk-highlight-wrapper"><mark class="risk-mark" style="background: {bg_color}; border-bottom: 2px solid {border_color}; padding: 1px 2px; border-radius: 3px; cursor: default;">{safe_original}</mark><span class="risk-tooltip"><span class="tooltip-header"><span style="display:inline-block;width:8px;height:8px;background:{border_color};border-radius:50%;margin-right:6px;"></span>{label}</span><div class="tooltip-section"><span class="tooltip-section-title">문제점</span><span class="tooltip-section-content tooltip-issue">{safe_explanation}</span></div><div class="tooltip-section"><span class="tooltip-section-title">이렇게 말해보세요</span><span class="tooltip-section-content tooltip-script">"{safe_script}"</span></div><div class="tooltip-section"><span class="tooltip-section-title">법적 근거</span><span class="tooltip-section-content"><strong>{safe_legal_ref}</strong></span></div></span></span>'''
